      ...
      <main className="mx-auto max-w-4xl px-6 py-12">
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-6">
          <div className="space-y-2">
            <label className="text-xs text-zinc-400">덱 이름</label>
            <input
              value={form.name}
              onChange={onChange("name")}
              className="w-full border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm"
              placeholder="Classic Miku"
            />
            {errors.includes("name") && (
              <p className="text-xs text-red-400">덱 이름을 입력해줘.</p>
            )}
          </div>

          <div className="space-y-2">
            <label className="text-xs text-zinc-400">설명 (선택)</label>
            <textarea
              value={form.description}
              onChange={onChange("description")}
              className="min-h-[96px] w-full border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm"
              placeholder="덱 소개를 적어줘"
            />
          </div>

          <div className="space-y-2">
            <label className="text-xs text-zinc-400">태그 (콤마로 구분)</label>
            <input
              value={form.tags}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs text-zinc-400">카드 선택</label>
            ...
            {errors.includes("cards") && (
              <p className="text-xs text-red-400">카드를 최소 1개 선택해줘.</p>
            )}
          </div>

          <button
            type="submit"
            className="rounded-full border border-emerald-400 px-4 py-2 text-sm text-emerald-300"
          >
            저장 시뮬레이션
          </button>

          {submitted && (
            <p className="text-xs text-emerald-300">입력 값이 유효해요!</p>
          )}
        </form>
      </main>
    </div>
